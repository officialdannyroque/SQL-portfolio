...
-- takes all order that had an exlucsion and unests them, assigning topping_id to their particular order_id
...
-- takes all order_ids with their exclusion ids and assigns them their topping name from pt table
exclusion_names AS (
	SELECT ee.order_id, 
	       STRING_AGG(pt.topping_name, ', ') AS exclusions
	FROM   exploded_exclusions ee
	LEFT JOIN pizza_toppings pt 
	ON     ee.topping_id = pt.topping_id
	GROUP BY 1
),

-- takes all the extras from the orders, unnests them and assigns them to their respective order_id
exploded_extras AS (
	SELECT order_id, 
...
SELECT co.order_id,
	   co.customer_id,
	   pn.pizza_name ||
	   CASE WHEN en.exclusions IS NOT NULL THEN ' - Exclude ' || en.exclusions ELSE '' END ||
	   CASE WHEN ex.extras IS NOT NULL THEN ' - Extra ' || ex.extras ELSE '' END AS order_description

FROM temp_customer_orders co
LEFT JOIN pizza_names pn 
ON co.pizza_id = pn.pizza_id
LEFT JOIN exclusion_names en 
ON co.order_id = en.order_id
LEFT JOIN extra_names ex 
