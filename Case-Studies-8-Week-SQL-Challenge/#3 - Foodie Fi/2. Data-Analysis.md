...
```sql
WITH customer_journeys AS (SELECT customer_id, plan_name,
								   LEAD(plan_name) OVER(PARTITION BY customer_id ORDER BY start_date) as next_plan
							FROM subscriptions s
							INNER JOIN plans p ON s.plan_id = p.plan_id)

SELECT next_plan AS plan_name, COUNT(*) AS conversion_count,
	   ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions)) AS conversion_percentage
FROM customer_journeys
